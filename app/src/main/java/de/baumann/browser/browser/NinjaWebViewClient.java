...
import de.baumann.browser.util.DecideUtil;
...
//    public void AddVideoInfo(String a, String b, String c) {
//        WeakReference<LinkedBlockingQueue> detectedTaskUrlQueueWeakReference = new
//                WeakReference<>(detectedTaskUrlQueue);
//        LinkedBlockingQueue detectedTaskUrlQueue = detectedTaskUrlQueueWeakReference.get();
//        if (detectedTaskUrlQueue != null)
//            detectedTaskUrlQueue.add(new DetectedVideoInfo(a, b, c));
//    }

    private final Set<Object> set = new HashSet<>();//添加webview url跳转信息

    @Override
    public void onLoadResource(WebView view, String url) {
        String nurl = view.getUrl();
        Log.i(TAG, "载入的url：  " + nurl + "  " + ninjaWebView.getTitle());
        String finalNurl = DecideUtil.ChangeUrlIsFaceBook(nurl);
        new Thread(() -> {
            if (set.add(finalNurl) && (DecideUtil.isYoutuBe(finalNurl) || DecideUtil.isVimeo(finalNurl) || DecideUtil.isTwitter(finalNurl) || DecideUtil.isInsTagram(finalNurl) || DecideUtil.isFacebook(finalNurl))) {
                YoutubeDLRequest request = new YoutubeDLRequest(finalNurl);
                request.addOption("-f", "b");
                VideoInfo streamInfo;
                String videoName;
                try {
                    streamInfo = YoutubeDL.getInstance().getInfo(request);
                    videoName = (streamInfo.getTitle().length() < 50) ? streamInfo.getTitle() : streamInfo.getTitle().substring(0, 49);
                    VideoDected.add(streamInfo);
//                    VideoDected.add(streamInfo.getUrl(), finalNurl, videoName, streamInfo.getThumbnail());
                    Log.i(TAG, "onLoadResource: 解析到加密链接 " + streamInfo.getUrl() + "   " + streamInfo.getTitle());
                } catch (YoutubeDLException | InterruptedException e) {
                    set.remove(finalNurl);
                    Log.i(TAG, "onLoadResource: 解析到加密链接 " + e);
                    e.printStackTrace();
                }
            } else if (url.contains("vimeo") || url.contains("twitter") || url.contains("instagram") || url.contains("facebook") || url.contains("twimg.com") || url.contains("akamaized.net") || url.contains("https://scontent-nrt1-1.xx.fbcdn.net")) {
            }
...
//            detectedTaskUrlQueue.add(new DetectedVideoInfo(request.getUrl().toString(), currentUrl, currentTitle));
//            Log.d("MainActivity", "shouldOverrideUrlLoading detectTaskUrlList.add(url):" + request.getUrl().toString());
//            return true;
//        }
